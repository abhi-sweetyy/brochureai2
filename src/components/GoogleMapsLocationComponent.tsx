...
const GoogleMapsLocationComponent: React.FC<GoogleMapsLocationProps> = ({
  ...
  const generateStaticMapUrl = (address: string): string => {
    ...
    // WICHTIG: Wir nutzen jetzt den korrekten scale-Parameter
    // und eine definierte Größe, die beim Einfügen besser funktioniert
    return `https://maps.googleapis.com/maps/api/staticmap?center=${encodedAddress}&zoom=14&size=800x450&scale=4&maptype=roadmap&markers=color:red%7C${encodedAddress}&key=${apiKey}`;
  };
        ...
        {error ? (
          ...
          <>
            {isGeneratingMap ? (
              ...
              <>
                {/* Mit fixierten Abmessungen um sicherzustellen, dass das Seitenverhältnis erhalten bleibt */}
                <div
                  className="w-full mb-4 overflow-hidden rounded-lg"
                  style={{
                    width: "100%",
                    position: "relative",
                    /* Erzwinge das 16:9 Verhältnis als Container */
