...
const DashboardHeader = ({ userEmail, isMenuOpen, setIsMenuOpen }: DashboardHeaderProps) => {
  ...
  useEffect(() => {
    ...
    const setupCreditsListener = async (currentSession: Session | null) => {
      if (currentSession?.user?.id) {
        ...
        creditListener = supabase.channel(`public:profiles:id=eq.${currentSession.user.id}`)
          .on(
            'postgres_changes',
            {
              event: 'UPDATE',
          ...
          .subscribe((status, err) => {
             if (status === 'SUBSCRIBED') {
               console.log('Subscribed to credit changes for user:', currentSession.user.id);
             }
                 ...
                 setTimeout(() => {
                   if (creditListener) {
                     ...
                     supabase.removeChannel(creditListener)
                       ...
                       .catch(error => {
                         console.warn('Error removing channel before reconnect:', error);
                       });
                   }
                 }, 5000);
               }
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="hidden sm:flex items-center px-3 py-1.5 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <span className="font-medium text-gray-900 text-sm whitespace-nowrap">{userCredits ?? '...'} {t('dashboard.credits')}</span>
          </div>
